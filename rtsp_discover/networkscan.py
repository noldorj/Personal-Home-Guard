...
async def ping_coroutine(cmd, ip):
    ...
    #print('cmd command  : {}'.format(cmd))
    
    #fileStdOut = open("pv-camFinder_stdout.log", "w")
    #fileStdErr = open("pv-camFinder_stderr.log", "w")   
    ...
    res = subprocess.call(cmd, shell=False, creationflags=CREATE_NO_WINDOW)   
    ...
    if res == 0:
        # Ping OK
        log.debug('ping ok ip: {}'.format(ip))
