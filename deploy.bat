...
pyinstaller initFormConfig.py --onefile --noconsole --hidden-import="pkg_resources.py2_warn" --hidden-import="googleapiclient" --hidden-import="apiclient" --onefile -n pv -y --add-data "pv-fb-cert.json;." --add-data "regions.json;." --add-data "pv-fb-cert.json;." --add-data "config.json;." --add-data "campainha.mp3;." --add-data "dlModels;." --distpath ../deploy-win  --add-binary "..\deploy-win\config\openvino\inference_engine\bin\intel64\Release;."  --add-binary "..\deploy-win\config\openvino\opencv\lib;." --hidden-import opencv --noconfirm --hidden-import matplotlib  
