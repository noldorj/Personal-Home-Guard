...
class OpenVinoPlugin():
    ...
    #self.labels_map = ["background", "person", "car", "bike"]
    ...
    def init(self, device, model_xml, model_bin, cpu_extension, plugin_dir):
    ...
    def getListBoxDetected(self, ipCam, device, frame, next_frame, nchw, exec_net, out_blob, input_blob, cur_request_id, next_request_id, prob_threshold, RES_X, RES_Y):


        
        prob_threshold_returned, xmin, xmax, ymin, ymax, det_label, class_id, label  = 0,0, 0, 0, 0, ' ', 0, ' '

        self.listObjectsTracking.clear()
        self.listRectanglesDetected.clear()
                ...
                for obj in res[0][0]:
                    ...
                    if obj[2] > prob_threshold:
                        ...
                        if det_label is 'person' or \
                            ...
                            self.listObjectsTracking.append(boxTracking)
                            self.listRectanglesDetected.append(box)


        listReturn = [frame, next_frame, cur_request_id, next_request_id, self.listRectanglesDetected, self.listObjectsTracking, prob_threshold_returned]
